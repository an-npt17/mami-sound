...
test "an override reaches the config and the rest of the preset stands" {
    const cfg = touchWith(.steady, 64, null, null, null, .{ false, false });
...
test "no overrides is the preset exactly" {
    try std.testing.expectEqual(touch, touchWith(null, null, null, null, null, .{ false, false }));
}
...
test "each plant is told to hold on its own" {
    ...
    const a_held = touchWith(null, null, null, null, null, .{ true, false });
    try std.testing.expect(a_held.hold);
    try std.testing.expect(!a_held.hold_bc);

    const both = touchWith(null, null, null, null, null, .{ true, true });
    try std.testing.expect(both.hold);
    try std.testing.expect(both.hold_bc);

    const neither = touchWith(null, null, null, null, null, .{ false, false });
...
test "a held plant drops its tap window, and only that plant's" {
    ...
    const b_held = touchWith(null, null, null, null, null, .{ false, true });
